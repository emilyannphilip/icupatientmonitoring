...
app.post('/api/users', async (req, res) => {
    ...
    try {
        ...
        db.run('INSERT INTO users (id, full_name, username, password_hash, designation, status, emp_id, email, profile_picture) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [id, fullName, username, hashedPassword, designation, status, empId, email, profilePicture], function (err) {
            if (err) {
...
app.put('/api/users/:id', async (req, res) => {
    ...
    try {
        if (password) {
            const salt = await bcryptjs_1.default.genSalt(10);
            const hashedPassword = await bcryptjs_1.default.hash(password, salt);
            db.run('UPDATE users SET full_name = ?, username = ?, password_hash = ?, designation = ?, status = ?, emp_id = ?, email = ?, profile_picture = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [fullName, username, hashedPassword, designation, status, empId, email, profilePicture, id], function (err) {
                if (err)
                    res.status(500).json({ message: 'Database error' });
                else
                    res.json({ id, fullName, username, designation, status, empId, email, profilePicture });
            });
        }
        else {
            db.run('UPDATE users SET full_name = ?, username = ?, designation = ?, status = ?, emp_id = ?, email = ?, profile_picture = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [fullName, username, designation, status, empId, email, profilePicture, id], function (err) {
                if (err)
                    res.status(500).json({ message: 'Database error' });
                else
                    res.json({ id, fullName, username, designation, status, empId, email, profilePicture });
